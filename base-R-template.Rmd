...
plotNormal <- function(repitions, mean, sd) {
  random_numbers <- rnorm(repitions, mean, sd)
  hist(random_numbers)
}
...
```{r}
set.seed(184)
plotNormal <- function(repitions, mean, sd, color = "blue") {
  random_numbers <- rnorm(repitions, mean, sd)
  hist(random_numbers, col = color, main = paste("Histogram from a Normal(", mean, ", ", sd, ") Distribution", sep = ""))
}
